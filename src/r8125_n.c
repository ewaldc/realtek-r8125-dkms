...
#define _R(NAME,FWNAME,MAC,RCR,MASK,HW_PHY_CFG,SW_RAMCODE_VER,CSI_FUN0_BYTE,ISR_VER,INT_MITI_VER) \
    { .name = NAME, .mcfg = MAC, .RCR_Cfg = RCR, .RxConfigMask = MASK, \
     .rtl8125_hw_phy_config = HW_PHY_CFG, .sw_ram_code_ver = SW_RAMCODE_VER, \
     .org_pci_offset_180 = (u8)CSI_FUN0_BYTE, .HwSuppIsrVer = (u8)ISR_VER, .HwSuppIntMitiVer = (u8)INT_MITI_VER }
...
static const struct {
        ...
        0xff7e5880, rtl8125_hw_phy_config_8125a_1, NIC_RAMCODE_VERSION_CFG_METHOD_2, 0x264, 1, 3),

        _R("RTL8125A", FIRMWARE_8125A_3,
        CFG_METHOD_3,
        Rx_Fetch_Number_8 | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_256 << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125a_2, NIC_RAMCODE_VERSION_CFG_METHOD_3, 0x264, 1, 3),

        _R("RTL8125B", "",
        CFG_METHOD_4,
        Rx_Fetch_Number_8 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_unlimited << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125b_1, NIC_RAMCODE_VERSION_CFG_METHOD_4, 0x214, 2, 4),

        _R("RTL8125B", FIRMWARE_8125B_2,
        CFG_METHOD_5,
        Rx_Fetch_Number_8 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_unlimited << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125b_2, NIC_RAMCODE_VERSION_CFG_METHOD_5, 0x214, 2, 4),

        _R("RTL8168KB", FIRMWARE_8125A_3,
        CFG_METHOD_6,
        Rx_Fetch_Number_8 | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_256 << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125a_2, NIC_RAMCODE_VERSION_CFG_METHOD_3, 0x264, 1, 3),

        _R("RTL8168KB", FIRMWARE_8125B_2,
        CFG_METHOD_7,
        ...
        0xff7e5880, rtl8125_hw_phy_config_8125bp_1, NIC_RAMCODE_VERSION_CFG_METHOD_8, 0x210, 4, 6),

        _R("RTL8125BP", FIRMWARE_8125BP_2,
        CFG_METHOD_9,
        Rx_Fetch_Number_8 | Rx_Close_Multiple | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_256 << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125bp_2, NIC_RAMCODE_VERSION_CFG_METHOD_9, 0x210, 4, 6),

        _R("RTL8125D", FIRMWARE_8125D_1,
        CFG_METHOD_10,
        Rx_Fetch_Number_8 | Rx_Close_Multiple | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_256 << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125d_1, NIC_RAMCODE_VERSION_CFG_METHOD_10, 0x210, 5, 7),

        _R("RTL8125D", FIRMWARE_8125D_2,
        CFG_METHOD_11,
        Rx_Fetch_Number_8 | Rx_Close_Multiple | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST_256 << RxCfgDMAShift),
        0xff7e5880, rtl8125_hw_phy_config_8125d_2, NIC_RAMCODE_VERSION_CFG_METHOD_11, 0x210, 5, 7),

        _R("Unknown", "",
        CFG_METHOD_UNKNOWN,
        (RX_DMA_BURST_512 << RxCfgDMAShift),
        0xff7e5880, NULL, 0, 0, 0, 0)
};
#undef _R
...
#endif
static int rtl8125_rx_interrupt(struct rtl8125_private *, struct rtl8125_rx_ring *, int);
static int rtl8125_tx_interrupt(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring, int budget);
static inline int rtl8125_tx_interrupt_with_vector(struct rtl8125_private *tp, const int message_id, int budget);
//static void rtl8125_wait_for_quiescence(struct rtl8125_private *tp);
...
rtl8125_disable_hw_interrupt_v2(struct rtl8125_private *tp, u32 message_id)
{
...
static int rtl8125_init_ring(struct rtl8125_private *tp);
...
static inline void rtl8125_mdio_set_page(struct rtl8125_private *tp, u16 value);
static inline void rtl8125_mdio_direct_write_phy_ocp(struct rtl8125_private *tp, u16 RegAddr,u16 value);
static inline u32 rtl8125_mdio_read(struct rtl8125_private *tp, u16 RegAddr);
...
#define rtl8125_set_speed rtl8125_set_speed_xmii
//static void rtl8125_set_speed(struct rtl8125_private *tp, u8 autoneg, u32 speed, u8 duplex, u64 adv);
...
rtl8125_get_desc_closed(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring)
{
	if (tp->HwSuppTxNoCloseVer == 3)
		return (RTL_R16(tp, ring->hw_clo_ptr_reg) - ring->BeginHwDesCloPtr)
		        & MAX_TX_NO_CLOSE_DESC_PTR_MASK_V2;
	return (RTL_R32(tp, ring->hw_clo_ptr_reg) - ring->BeginHwDesCloPtr)
		& MAX_TX_NO_CLOSE_DESC_PTR_MASK_V4;
}

#ifdef ENABLE_R8125_PROCFS
static u32
rtl8125_get_hw_clo_ptr(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring)
{
        return (tp->HwSuppTxNoCloseVer == 3) ?  RTL_R16(tp, ring->hw_clo_ptr_reg) :
                                                RTL_R32(tp, ring->hw_clo_ptr_reg);
}
#endif
...
static int proc_get_driver_variable(struct seq_file *m, void *v)
{
        ...
        seq_printf(m, "Dash_enabled\t0x%x\n", tp->dash_enabled);
        seq_printf(m, "Dash_printer_enabled\t0x%x\n", tp->dash_printer_enabled);
        seq_printf(m, "HwSuppOcpChannelVer\t0x%x\n", tp->HwSuppOcpChannelVer);
        seq_printf(m, "AllowAccessDashOcp\t0x%x\n", tp->AllowAccessDashOcp);
        ...
        seq_printf(m, "hw_clo_ptr_reg0\t0x%x\n", rtl8125_get_hw_clo_ptr(tp, &tp->tx_ring[0]));
        ...
        seq_printf(m, "hw_clo_ptr_reg1\t0x%x\n", rtl8125_get_hw_clo_ptr(tp, &tp->tx_ring[1]));
...
//#define rtl8125_delay_timer(delay, post_delay) (post_delay << 16) | (delay & 0xFFFF)

/*
static bool
rtl8125_timed_wait_debug(void __iomem *addr, bool b, enum RTL8125_register_content flag, int delay, int count)
{
	static int cnt0=0, cnt1=10000, cnt2=0, cnt3=0, cnt4=0; 
	cnt0++; 
	for (int i = 0; i < count; i++) {
		udelay(delay);
			...
			if (i > cnt2) cnt2 = i;
			cnt3 += i;
			if (cnt0 % 1000) printk(KERN_INFO "r8125 - loop_wait: 0:%d, 1:%d, 2:%d, 3:%d, 4:%d\n");
                        return 1;
		}
        }
	cnt4++;
...
static __always_inline void rtl8125_mdio_set_page(struct rtl8125_private *tp, u16 value)
{
        tp->cur_page = value;
}
...
{
	//return (page == 0) ? OCP_STD_PHY_BASE_PAGE + ((reg / 8) << 4) + ((reg % 8) << 1): 0;
	return (page == 0) ? OCP_STD_PHY_BASE_PAGE + (reg << 1): 0;
}

static void 
rtl8125_mdio_direct_write_phy_ocp(struct rtl8125_private *tp, u16 reg, u16 value)
{
#ifdef ENABLE_RTL_TOOL
...
static u32 rtl8125_mdio_direct_read_phy_ocp(struct rtl8125_private *tp, u16 reg)
{
#ifdef ENABLE_RTL_TOOL
        if (unlikely(tp->rtk_enable_diag))
                return 0xffffffff;
...
static inline void rtl8125_mdio_write(struct rtl8125_private *tp, u16 reg, u16 value)
{
#ifdef ENABLE_RTL_TOOL
...
static inline u32 rtl8125_mdio_read(struct rtl8125_private *tp, u16 reg)
{
        return rtl8125_mdio_direct_read_phy_ocp(tp, map_phy_ocp_addr(tp->cur_page, reg));
}

/*
static inline void rtl8125_clear_eth_phy_bit(struct rtl8125_private *tp, u8 reg, u16 mask)
{
        rtl8125_mdio_write(tp, reg, rtl8125_mdio_read(tp, reg) & ~mask);
}

static inline void rtl8125_set_eth_phy_bit(struct rtl8125_private *tp, u8 reg, u16 mask)
{
        rtl8125_mdio_write(tp, reg, rtl8125_mdio_read(tp, reg) | mask);
}
*/
...
static inline void
rtl8125_clear_and_set_eth_phy_ocp_bit(struct rtl8125_private *tp, u16 reg, u16 clearmask, u16 setmask)
{
        rtl8125_mdio_write(tp, reg, 
		(rtl8125_mdio_read(tp, reg) & ~clearmask) | setmask);
}

static inline void
rtl8125_clear_eth_phy_ocp_bit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
        rtl8125_mdio_write(tp, addr, 
		rtl8125_mdio_read(tp, addr) & ~mask);
}

static inline void
rtl8125_set_eth_phy_ocp_bit(struct rtl8125_private *tp,  u16 addr, u16 mask)
{
        rtl8125_mdio_write(tp, addr, 
		rtl8125_mdio_read(tp, addr) | mask);
}

static inline void rtl8125_mac_ocp_write(struct rtl8125_private *tp, u16 reg_addr, u16 value)
{
        WARN_ON_ONCE(reg_addr & 0x0001); // Uneven register address means trouble
        //WARN_ONCE(reg_addr & 0xffff0001, "Invalid ocp reg %x!\n", reg_addr);
	RTL_W32(tp, MACOCP, OCPR_Write | (reg_addr << 15) | value);
}
...
void
rtl8125_clear_set_mac_ocp_bit(struct rtl8125_private *tp, u16 addr, u16 clearmask, u16 setmask)
{
...
/*
static inline void
rtl8125_clear_mac_ocp_bit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
        rtl8125_clear_set_mac_ocp_bit(tp, addr, mask, 0);
}

static inline void
rtl8125_set_mac_ocp_bit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
...
u32 inline rtl8125_ocp_read(struct rtl8125_private *tp, u16 addr, u8 len)
{
        u32 base_address;
        ...
        if (tp->HwSuppOcpChannelVer == 2)
                ...
                base_address = RTL8168FP_OOBMAC_BASE;
        else return 0;

        return rtl8125_ocp_read_with_oob_base_address(tp, addr, len, base_address);
}
...
void __always_inline rtl8125_ocp_write(struct rtl8125_private *tp, u16 addr, u8 len, u32 value)
{
        u32 base_address;
        if (!tp->AllowAccessDashOcp)
                return;
        ...
        rtl8125_eri_write_with_oob_base_address(tp, addr, len, value, ERIAR_OOB, base_address);
}

void rtl8125_oob_mutex_lock(struct rtl8125_private *tp)
{
        u8 reg_16, reg_a0;
        u32 wait_cnt_0 = 0, wait_Cnt_1;

        rtl8125_ocp_write(tp, 0x114, 1, BIT_0);
        reg_16 = rtl8125_ocp_read(tp, 0x110, 1);
        while(reg_16) {
                reg_a0 = rtl8125_ocp_read(tp, 0x11C, 1);
                if (reg_a0) {
                        rtl8125_ocp_write(tp, 0x114, 1, 0x00);
                        reg_a0 = rtl8125_ocp_read(tp, 0x11C, 1);
                        wait_Cnt_1 = 0;
                        while(reg_a0) {
                                reg_a0 = rtl8125_ocp_read(tp, 0x11C, 1);
...
void inline rtl8125_oob_mutex_unlock(struct rtl8125_private *tp)
{
        //ocp_reg_mutex_oob = 0x110;
        rtl8125_ocp_write(tp, 0x11C, 1, BIT_0);
        rtl8125_ocp_write(tp, 0x114, 1, 0x00);
}
...
{
        return rtl8125_ocp_read(tp, OCP_REG_FIRMWARE_MAJOR_VERSION, 4);
}
...
{
        return (_rtl8125_check_dash(tp) && (rtl8125_get_dash_fw_ver(tp) != 0)) ? 1 : 0;

}

inline void rtl8125_dash2_disable_tx(struct rtl8125_private *tp)
{
        u16 WaitCnt;
        u8 TmpUchar;
...
inline void rtl8125_dash2_disable_rx(struct rtl8125_private *tp)
{
...
static void rtl8125_dash2_disable_txrx(struct rtl8125_private *tp)
{
        rtl8125_dash2_disable_tx(tp);
        rtl8125_dash2_disable_rx(tp);
}

static void rtl8125_wait_dash_fw_ready(struct rtl8125_private *tp)
{
...
rtl8125_notify_dash_oob_cmac(struct rtl8125_private *tp, u32 cmd)
{
        ...
        rtl8125_ocp_write(tp, 0x30, 4, rtl8125_ocp_read(tp, 0x30, 4) | BIT_0);
}
...
{
        return (HW_DASH_SUPPORT_IPC2(tp)) ?
                rtl8125_notify_dash_oob_ipc2(tp, cmd) :
                rtl8125_notify_dash_oob_cmac(tp, cmd);
}
...
static void rtl8125_driver_stop(struct rtl8125_private *tp)
{
        ...
        if (tp->dash_enabled && HW_DASH_SUPPORT_CMAC(tp))
...
void rtl8125_ephy_write(struct rtl8125_private *tp, u8 reg, u16 value)
{
...
u16 rtl8125_ephy_read(struct rtl8125_private *tp, int reg)
{
...
static inline void ClearAndSetPCIePhyBit(struct rtl8125_private *tp, u8 addr, u16 clearmask, u16 setmask)
{
        rtl8125_ephy_write(tp, addr, (rtl8125_ephy_read(tp, addr) & ~clearmask) | setmask);
}

static inline void ClearPCIePhyBit(struct rtl8125_private *tp, u8 addr, u16 mask)
{
        rtl8125_ephy_write(tp, addr, rtl8125_ephy_read(tp, addr) & ~mask);
}

static inline void SetPCIePhyBit(struct rtl8125_private *tp, u8 addr, u16 mask)
{
        rtl8125_ephy_write(tp, addr, rtl8125_ephy_read(tp, addr) | mask);
}

static u32
rtl8125_csi_fun_read(struct rtl8125_private *tp, u8 func, u32 addr)
{
        u32 rc; 
        func &= 0x07;
...
static void
rtl8125_csi_fun_write(struct rtl8125_private *tp, u8 func, u32 addr, u32 value)
{
...
static __always_inline u32
rtl8125_csi_read(struct rtl8125_private *tp, u32 addr)
{
        return rtl8125_csi_fun_read(tp, (u8)0, addr);
}

static __always_inline void
rtl8125_csi_write(struct rtl8125_private *tp, u32 addr, u32 value)
{
...
static inline void
rtl8125_disable_rxdvgate(struct rtl8125_private *tp)
{
        RTL_W8(tp, 0xF2, RTL_R8(tp, 0xF2) & ~BIT_3);
        mdelay(2);
}
...
{
        if (tp->HwSuppCheckPhyDisableModeVer == 3 && 
                (!(rtl8125_mac_ocp_read(tp, 0xDC04) & BIT_13))) {
                dprintk("gpio is low.\n");
                return TRUE;
        }
        return FALSE;
...
{
        if (tp->HwSuppCheckPhyDisableModeVer == 3 && (RTL_R8(tp, 0xF2) & BIT_5)) {
...
static void rtl8125_stop_all_request(struct rtl8125_private *tp)
{
        RTL_W8(tp, ChipCmd, RTL_R8(tp, ChipCmd) | StopReq);
        if (tp->mcfg <= CFG_METHOD_3 || tp->mcfg == CFG_METHOD_6)
                for (int i = 0; i < 20; i++) {
                        udelay(10);
...
{
	bool isNotMethod2_3_6 = (tp->mcfg >= CFG_METHOD_4 && tp->mcfg != CFG_METHOD_6);
        int i;
...
static __always_inline void rtl8125_clear_hw_isr_v2(struct rtl8125_private *tp,
                u32 message_id)
{
...
static void
rtl8125_nic_reset(struct rtl8125_private *tp)
{
...
static void
rtl8125_hw_clear_int_miti(struct rtl8125_private *tp)
{
...
void
rtl8125_hw_set_timer_int_8125(struct rtl8125_private *tp, u32 message_id, u8 timer_intmiti_val)
{
        switch (tp->HwSuppIntMitiVer) {
        case 4:
        case 6:
...
void
rtl8125_hw_reset(struct rtl8125_private *tp)
{
...
rtl8125_xmii_link_ok(struct rtl8125_private *tp)
{
...
static void
rtl8125_xmii_reset_enable(struct rtl8125_private *tp)
{
...
static void rtl8125_enable_eee_plus(struct rtl8125_private *tp)
{
        rtl8125_set_mac_ocp_bit(tp, 0xE080, BIT_1);
}

static void rtl8125_disable_eee_plus(struct rtl8125_private *tp)
{
...
static void
rtl8125_link_on_patch(struct rtl8125_private *tp, struct net_device *dev)
{
...
static void
rtl8125_link_down_patch(struct rtl8125_private *tp, struct net_device *dev)
{
        tp->phy_reg_aner = 0;
        tp->phy_reg_anlpar = 0;
        tp->phy_reg_gbsr = 0;
        ...
        if (likely(tp->mcfg <= CFG_METHOD_9))
        ...
        rtl8125_tx_clear(tp);
        rtl8125_rx_clear(tp);
        rtl8125_init_ring(tp);
        rtl8125_enable_hw_linkchg_interrupt(tp);

        //rtl8125_set_speed(tp, tp->autoneg, tp->speed, tp->duplex, tp->advertising);
...
static inline void
_rtl8125_check_link_status(struct rtl8125_private *tp, struct net_device *dev)
{
        if (rtl8125_xmii_link_ok(tp)) {
                rtl8125_link_on_patch(tp, dev);
...
static void
rtl8125_set_link_option(struct rtl8125_private *tp, u8 autoneg, u32 speed,
        u8 duplex, enum rtl8125_fc_mode fc)
{
...
static void
rtl8125_disable_ocp_phy_power_saving(struct rtl8125_private *tp)
{
...
static void
rtl8125_set_pci_99_180_exit_driver_para(struct rtl8125_private *tp)
{
...
static void
rtl8125_enable_magic_packet(struct rtl8125_private *tp)
{
        if (tp->HwSuppMagicPktVer == WAKEUP_MAGIC_PACKET_V3)
                rtl8125_set_mac_ocp_bit(tp, 0xC0B6, BIT_0);
}

static inline void
rtl8125_disable_magic_packet(struct rtl8125_private *tp)
{
        if (tp->HwSuppMagicPktVer == WAKEUP_MAGIC_PACKET_V3)
...
static void
rtl8125_disable_linkchg_wakeup(struct rtl8125_private *tp)
{
        RTL_W8(tp, Config3, RTL_R8(tp, Config3) & ~LinkUp);
...
rtl8125_set_hw_wol(struct net_device *dev, u32 wolopts)
{
        ...
        if (tp->HwSuppLinkChgWakeUpVer == 3) {
                if (wolopts & WAKE_PHY)
...
rtl8125_keep_wol_link_speed(struct net_device *dev, u8 from_suspend)
{
        ...
        if ((from_suspend && !rtl8125_xmii_link_ok(tp)) ||
...
#ifdef ENABLE_DASH_SUPPORT
        if (tp->wol_enabled == WOL_ENABLED || tp->EnableKCPOffload || tp->dash_enabled) {
#else
...
static int
rtl8125_set_speed_xmii(struct rtl8125_private *tp,
                       u8 autoneg,
                       u32 speed,
                       u8 duplex,
                       u64 adv)
...
static inline u32 rtl8125_get_tx_csum(struct net_device *dev)
{
...
rtl8125_rx_desc_opts1(struct rtl8125_private *tp, struct RxDesc *desc)
{
        ...
        if (likely(tp->InitRxDescType == RX_DESC_RING_TYPE_4))
...
static inline void
rtl8125_clear_rx_desc_opts2(struct rtl8125_private *tp, struct RxDesc *desc)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static int rtl_get_eeprom(struct net_device *dev, struct ethtool_eeprom *eeprom, u8 *buf)
{
        ...
        if (tp->eeprom_type == EEPROM_TYPE_NONE) {
                netdev_printk(KERN_DEBUG, dev, "No EEPROM detected\n");
                return -EOPNOTSUPP;
...
{
        return (tp->mcfg <= CFG_METHOD_9 && (rtl8125_mdio_direct_read_phy_ocp(tp, 0xA430) & BIT_15));
                //case CFG_METHOD_10...CFG_METHOD_11:
}
...
static int rtl8125_enable_eee(struct rtl8125_private *tp)
{
	...
	if (tp->mcfg >= CFG_METHOD_4 && tp->mcfg != CFG_METHOD_6)
                rtl8125_set_mac_ocp_bit(tp, 0xE040, (BIT_1|BIT_0));
	else  // CFG_METHOD_2/3/6   
        ...
        rtl8125_clear_and_set_eth_phy_ocp_bit(tp, 0xA5D0,
        	MDIO_EEE_100TX | MDIO_EEE_1000T, eee_adv_t);
...
static int
rtl_ethtool_set_eee(struct net_device *dev, struct ethtool_eee *edata)
{
        struct rtl8125_private *tp = netdev_priv(dev);
        struct ethtool_eee *eee = &tp->eee;
        u64 advertising;
...
#ifdef ENABLE_DASH_SUPPORT
        if (!HW_HAS_WRITE_PHY_MCU_RAM_CODE(tp) || tp->dash_enabled)
#else
...
{
        printk(KERN_INFO "Realtek %s Ethernet controller for %s (mcfg = %04d)\n",
                MODULENAME, rtl_chip_info[tp->chipset].name, rtl_chip_info[tp->chipset].mcfg);
}
...
rtl8125_clear_phy_ups_reg(struct rtl8125_private *tp)
{
        ...
        case CFG_METHOD_4:
        case CFG_METHOD_5:
        case CFG_METHOD_7...CFG_METHOD_11:
...
static void
rtl8125_wait_phy_ups_resume(struct rtl8125_private *tp, u16 PhyState)
{
        int i;
        for (i=0; i< 100; i++) {
...
static void
rtl8125_exit_oob(struct rtl8125_private *tp)
{
        u16 data16;
        rtl8125_disable_rx_packet_filter(tp);

#ifdef ENABLE_DASH_LIB_SUPPORT
        if (tp->dash_enabled) {
                if (tp->AllowAccessDashOcp)
...
{
        if (FALSE == HW_SUPPORT_MAC_MCU(tp) || entry == NULL || entry_cnt == 0)
                return;

        //if (tp->MacMcuPageSize > 0)
                _rtl8125_write_mac_mcu_ram_code_with_page(tp, entry, entry_cnt, MacMcuPageSize);
        //else    _rtl8125_write_mac_mcu_ram_code(tp, entry, entry_cnt);
}
...
#endif
	rtl_chip_info[tp->chipset].rtl8125_hw_phy_config(tp);
...
rtl8125_init_software_variable(struct net_device *dev)
{
        ...
        tp->HwPkgDet = (rtl8125_mac_ocp_read(tp, 0xDC00) >> 3) & 0x07;
        ...
        tp->HwPcieSNOffset = (tp->mcfg <= CFG_METHOD_7) ? 0x16C: 0x168;
        ...
        if (aspm) {
                ...
                tp->org_pci_offset_180 = rtl8125_csi_fun0_read_byte(tp, 
                        rtl_chip_info[tp->chipset].org_pci_offset_180);
                /*
                switch (tp->mcfg) {
                case CFG_METHOD_2:
                case CFG_METHOD_3:
        ...
        if ((tp->mcfg | CFG_METHOD_9) != CFG_METHOD_9)
                tp->HwSuppMagicPktVer = WAKEUP_MAGIC_PACKET_V3;
        else // CFG_METHOD_8 and CFG_METHOD_9
        ...
        tp->sw_ram_code_ver = rtl_chip_info[tp->chipset].sw_ram_code_ver;
        ...
        //init interrupt
        tp->HwSuppIsrVer = rtl_chip_info[tp->chipset].HwSuppIsrVer;
        tp->HwCurrIsrVer = tp->HwSuppIsrVer;
        if (tp->HwCurrIsrVer > 1) {
                if ((tp->irq_nvecs <= 1) ||  // implies !RTL_FEATURE_MSIX
                //if (!(tp->features & RTL_FEATURE_MSIX) ||
#ifdef ENABLE_DASH_SUPPORT
                                (tp->dash_enabled && HW_DASH_SUPPORT_TYPE_4(tp)) ||
#endif
        ...
        case CFG_METHOD_7 ... CFG_METHOD_11:
                ...
                if (tp->HwCurrIsrVer > 1) {
                        ...
                        if (tp->irq_nvecs < tp->num_rx_rings)
                                tp->num_rx_rings = 1;

                        if (tp->num_rx_rings >= 2) {
                                tp->EnableRss = 1;
                                printk(KERN_INFO "r8125: RSS enabled (%d queues)\n", tp->num_rx_rings);
                        }
                }
#endif
                break;
        ...
        //rtl8125_set_ring_size(tp, NUM_RX_DESC, NUM_TX_DESC);
       	...
       	tp->HwSuppIntMitiVer = rtl_chip_info[tp->chipset].HwSuppIntMitiVer;
        /*
        tp->HwSuppTcamVer = 1;
	...
	tp->rtl8125_rx_config = rtl_chip_info[tp->chipset].RCR_Cfg;
        ...
        if (tp->mcfg >= CFG_METHOD_4 && HW_SUPP_PHY_LINK_SPEED_2500M(tp)) {
...
static int
rtl8125_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
        struct rtl8125_private *tp = netdev_priv(dev);
...
#ifdef ENABLE_PTP_SUPPORT
        case SIOCSHWTSTAMP:
                return (tp->EnablePtp)? rtl8125_set_tstamp(tp, ifr): -EOPNOTSUPP;
        case SIOCGHWTSTAMP:
                return (tp->EnablePtp)? rtl8125_get_tstamp(tp, ifr): -EOPNOTSUPP;
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,15,0)
#ifdef ENABLE_DASH_LIB_SUPPORT
        case SIOCDEVPRIVATE_RTLDASH:
                ...
                if (!capable(CAP_NET_ADMIN))
                        return -EPERM;
                return rtl8125_dash_ioctl(dev, ifr);
#endif
...
static void
rtl8125_phy_power_up(struct rtl8125_private *tp)
{
        if (rtl8125_is_in_phy_disable_mode(tp))
                return;
...
rtl8125_init_board(struct pci_dev *pdev, struct net_device **dev_out,
                   void __iomem **ioaddr_out)
{
        ...
        /* Ewald: By rejecting unknown chips, we can eliminate countless tests for valid tp->mcfg */
...
{
        struct pci_dev *pdev = tp->pci_dev;
        unsigned int msi, hw_supp_irq_nvecs;
        int nvecs = 1;

        hw_supp_irq_nvecs = (tp->mcfg <= CFG_METHOD_3) ? R8125_MAX_MSIX_VEC_8125A
                : R8125_MAX_MSIX_VEC;
        ...
        case CFG_METHOD_5:
        case CFG_METHOD_7:
                tp->max_irq_nvecs = hw_supp_irq_nvecs;
...
static void rtl8125_disable_msi(struct pci_dev *pdev, struct rtl8125_private *tp)
{
        ...
        //if (likely(tp->features & (RTL_FEATURE_MSI | RTL_FEATURE_MSIX)))
        if (likely(tp->irq_nvecs >= 1)) // impliesRTL_FEATURE_MSIX
                pci_free_irq_vectors(pdev);
#else
        //if (tp->features & (RTL_FEATURE_MSIX))
        if (tp->nvecs > 1) // impliesRTL_FEATURE_MSIX
                pci_disable_msix(pdev);
        else  if (tp->nvecs = 1)   // implies RTL_FEATURE_MSI  //if (tp->features & (RTL_FEATURE_MSI))
...
static int rtl8125_poll(struct napi_struct *napi, int budget)
{
        struct r8125_napi *r8125napi = container_of(napi, struct r8125_napi, napi);
        struct rtl8125_private *tp = r8125napi->priv;
        int i, work_done = 0;

        for (i = 0; i < tp->num_tx_rings; i++)
                rtl8125_tx_interrupt(tp, &tp->tx_ring[i], budget);
        for (i = 0; i < tp->num_rx_rings; i++)
                work_done += rtl8125_rx_interrupt(tp, &tp->rx_ring[i], budget);
...
static int rtl8125_poll_msix_ring(struct napi_struct *napi, int budget)
{
        struct r8125_napi *r8125napi = container_of(napi, struct r8125_napi, napi);
        struct rtl8125_private *tp = r8125napi->priv;
        int work_done = 0;
        ...
        work_done += rtl8125_rx_interrupt(tp, &tp->rx_ring[message_id], budget);
        if (work_done < budget) {
#ifdef ENABLE_DASH_LIB_SUPPORT
                if (tp->dash_enabled && message_id == 0)
                        HandleDashInterrupt(tp->dev);
#endif
...
static int rtl8125_poll_msix_tx(struct napi_struct *napi, int budget)
{
        struct r8125_napi *r8125napi = container_of(napi, struct r8125_napi, napi);
        struct rtl8125_private *tp = r8125napi->priv;
        const int message_id = r8125napi->index;
...
static int rtl8125_poll_msix_rx(struct napi_struct *napi, int budget)
{
        struct r8125_napi *r8125napi = container_of(napi, struct r8125_napi, napi);
        struct rtl8125_private *tp = r8125napi->priv;
        const int message_id = r8125napi->index;
        int work_done = rtl8125_rx_interrupt(tp, &tp->rx_ring[message_id], budget);
...
static void rtl8125_enable_napi(struct rtl8125_private *tp)
{
        for (int i = 0; i < tp->irq_nvecs; i++)
                napi_enable(&tp->r8125napi[i].napi);
}

static void rtl8125_disable_napi(struct rtl8125_private *tp)
{
        for (int i = 0; i < tp->irq_nvecs; i++)
                napi_disable(&tp->r8125napi[i].napi);
}
...
static void rtl8125_init_napi(struct rtl8125_private *tp)
{
        for (int i=0; i<tp->irq_nvecs; i++) {
                ...
                if (tp->irq_nvecs > 1) { // implies RTL_FEATURE_MSIX && tp->HwCurrIsrVer > 1
                ...
                netif_napi_add_weight(tp->dev, &r8125napi->napi, poll, R8125_NAPI_WEIGHT);
#endif
                r8125napi->priv = tp;
                r8125napi->index = i;
        }
...
rtl8125_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
        ...
        tp->fw_name = rtl_chip_info[tp->chipset].fw_name;
...
#ifdef ENABLE_PAGE_REUSE
static inline unsigned int rtl8125_rx_page_order()
{
        unsigned truesize = SKB_DATA_ALIGN(sizeof(struct skb_shared_info)) +
                            SKB_DATA_ALIGN(RX_BUF_SIZE + R8125_RX_ALIGN);
                      ...
                      struct net_device *dev)
{
        ...
        tp->rx_buf_sz = RX_BUF_SIZE; //SKB_DATA_ALIGN(RX_BUF_SIZE);
#endif //ENABLE_RX_PACKET_FRAGMENT
#ifdef ENABLE_PAGE_REUSE
        tp->rx_buf_page_order = rtl8125_rx_page_order();
...
static int rtl8125_alloc_irq(struct rtl8125_private *tp)
{
        ...
        for (i=0; i<tp->irq_nvecs; i++) {
                ...
                snprintf(irq->name, len, "%s-%d", dev->name, i);
                rc = pci_request_irq(tp->pci_dev, i, irq->handler, NULL, r8125napi, irq->name);
                if (rc)
                        break;
        ...
        } else {  //tp->irq_nvecs <= 1 implies !RTL_FEATURE_MSIX
                irq = &tp->irq_tbl[0];
                irq->handler = rtl8125_interrupt;
                r8125napi = &tp->r8125napi[0];
                snprintf(irq->name, len, "%s-0", dev->name);
                irq->vector = dev->irq;
                if (!pci_dev_msi_enabled(tp->pci_dev))  // implies !RTL_FEATURE_MSI
                        irq_flags |= IRQF_SHARED;
                //irq_flags |= (tp->features & (RTL_FEATURE_MSI | RTL_FEATURE_MSIX)) ? 0 : IRQF_SHARED;
                rc = request_irq(irq->vector, irq->handler, irq_flags, irq->name, r8125napi);
...
static int rtl8125_alloc_desc(struct rtl8125_private *tp)
{
        ...
        for (int i = 0; i < tp->num_tx_rings; i++) {
                ...
                tx_ring->TxDescAllocSize = (NUM_TX_DESC + 1) * sizeof(struct TxDesc);
                tx_ring->TxDescArray = dma_alloc_coherent(device, tx_ring->TxDescAllocSize,
                                                       &tx_ring->TxPhyAddr, GFP_KERNEL);
                if (!tx_ring->TxDescArray)
                        return -1;
        }
        for (int i = 0; i < tp->num_rx_rings; i++) {
                rx_ring = &tp->rx_ring[i];
                rx_ring->RxDescAllocSize = (NUM_RX_DESC + 1) * tp->RxDescLength;
                rx_ring->RxDescArray = dma_alloc_coherent(device, rx_ring->RxDescAllocSize,
                                                       &rx_ring->RxPhyAddr, GFP_KERNEL);

                if (!rx_ring->RxDescArray)
                        return -1;
        }
        return 0;
}
...
static void rtl8125_free_desc(struct rtl8125_private *tp)
{
        ...
        for (int i = 0; i < tp->num_tx_rings; i++) {
                tx_ring = &tp->tx_ring[i];
                if (tx_ring->TxDescArray) {
                        dma_free_coherent(device, tx_ring->TxDescAllocSize, 
                                tx_ring->TxDescArray, tx_ring->TxPhyAddr);
                        tx_ring->TxDescArray = NULL;
                }
        }
        for (int i = 0; i < tp->num_rx_rings; i++) {
                rx_ring = &tp->rx_ring[i];
                if (rx_ring->RxDescArray) {
                        dma_free_coherent(device, rx_ring->RxDescAllocSize,
                                rx_ring->RxDescArray, rx_ring->RxPhyAddr);
                        rx_ring->RxDescArray = NULL;
                }
        }
}
...
{
        u32 csi_tmp = rtl8125_csi_read(tp, 0x70c) & 0x00ffffff;
        ...
        rtl8125_csi_write(tp, 0x70c, csi_tmp | ((u32)setting) << 24);
}
...
static void
rtl8125_hw_set_rx_packet_filter(struct rtl8125_private *tp, struct net_device *dev)
{
...
{
        rtl8125_hw_set_rx_packet_filter(netdev_priv(dev), dev);
}

void
rtl8125_set_rx_q_num(struct rtl8125_private *tp, unsigned int num_rx_queues)
{
        u16 rx_q_num = ((u16)ilog2(num_rx_queues) & (BIT_0 | BIT_1 | BIT_2)) << 2;
        u16 q_ctrl = (RTL_R16(tp, Q_NUM_CTRL_8125) & ~(BIT_2 | BIT_3 | BIT_4)) | rx_q_num;
        RTL_W16(tp, Q_NUM_CTRL_8125, q_ctrl);
}

void
rtl8125_set_tx_q_num(struct rtl8125_private *tp, unsigned int num_tx_queues)
{
        rtl8125_clear_set_mac_ocp_bit(tp, 0xE63E, (u16)(BIT_11 | BIT_10),
                (u16)((ilog2(num_tx_queues) & 0x03) << 10));
}
...
{
        if (tp->mcfg >= CFG_METHOD_2 && tp->mcfg != CFG_METHOD_6)
...
rtl8125_hw_config(struct rtl8125_private *tp, struct net_device *dev)
{
	...
	/* TCAM */
        ...
        if (tp->mcfg == CFG_METHOD_4 || tp->mcfg == CFG_METHOD_5 || tp->mcfg == CFG_METHOD_7)
        ...
        if (tp->mcfg == CFG_METHOD_2 || tp->mcfg == CFG_METHOD_3 || tp->mcfg == CFG_METHOD_6)
	...
	if (unlikely(tp->dash_enabled))
        ...
        */
        ...
        if (tp->use_timer_interrupt && (tp->HwSuppIntMitiVer > 3) &&
                //(tp->HwCurrIsrVer > 1) && (tp->features & RTL_FEATURE_MSIX)) {
                tp->irq_nvecs > 1) {  // implies RTL_FEATURE_MSIX
                for (int i = 0; i < tp->irq_nvecs; i++)
        ...
        RTL_W16(tp, RxMaxSize, SKB_DATA_ALIGN(RX_BUF_SIZE));
        ...
        rtl8125_hw_set_rx_packet_filter(tp, dev);

#ifdef ENABLE_DASH_SUPPORT
        if (tp->dash_enabled && !tp->dash_printer_enabled)
                NICChkTypeEnableDashInterrupt(tp);
#endif
...
static void
rtl8125_hw_start(struct rtl8125_private *tp)
{
#ifdef ENABLE_LIB_SUPPORT
...
static inline void
rtl8125_set_desc_dma_addr(struct rtl8125_private *tp, struct RxDesc *desc, dma_addr_t mapping)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
inline void
rtl8125_mark_to_asic(struct rtl8125_private *tp, struct RxDesc *desc)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
inline void
rtl8125_map_and_mark_to_asic(struct rtl8125_private *tp, struct RxDesc *desc, 
                             dma_addr_t mapping)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3) {
...
static inline void
rtl8125_map_to_asic(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring, struct RxDesc *desc,
                    dma_addr_t mapping, const u32 cur_rx)
{
...
static __always_inline rtl8125_free_rx_page(struct rtl8125_private *tp, struct rtl8125_rx_buffer *rxb)
{
        if (!rxb->page)
                return;
...
static u32
rtl8125_rx_fill(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring,
        u32 start, u32 end, u8 in_intr)
{
        ...
        for (cur = start; end - cur > 0; cur++) {
                ...
                dma_sync_single_range_for_device(device, rxb->dma, rxb->page_offset, DMA_FROM_DEVICE);
                rtl8125_map_to_asic(tp, ring, rtl8125_get_rxdesc(tp, ring->RxDescArray, i),
                                    rxb->dma + rxb->page_offset, i);
        }
        return cur - start;
}
...
/*
static __always_inline void rtl8125_free_rx_skb(struct rtl8125_private *tp,
                    ...
                    const u32 cur_rx)
{
        ...
        dma_unmap_single(&pdev->dev, ring->RxDescPhyAddr[cur_rx], RX_BUF_SIZE, DMA_FROM_DEVICE);
...
static inline void
rtl8125_make_unusable_by_asic(struct rtl8125_private *tp, struct RxDesc *desc)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3) {
...
rtl8125_alloc_rx_skb(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring, struct sk_buff **sk_buff,
        struct RxDesc *desc, const u32 cur_rx, u8 in_intr)
{
        struct sk_buff *skb;
        dma_addr_t mapping;
        struct device *device = tp->device;
        ...
        if (in_intr)
                skb = RTL_ALLOC_SKB_INTR(&tp->r8125napi[ring->index].napi, RX_BUF_SIZE + R8125_RX_ALIGN);
        else
                skb = dev_alloc_skb(RX_BUF_SIZE + R8125_RX_ALIGN);
        ...
        mapping = dma_map_single(device, skb->data, RX_BUF_SIZE, DMA_FROM_DEVICE);
        ...
        *sk_buff = skb;
        rtl8125_map_to_asic(tp, ring, desc, mapping, cur_rx);
        return 0;
...
_rtl8125_rx_clear(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring)
{
	...
	for (int i = 0; i < ring->num_rx_desc; i++) {
		sk_buff = ring->Rx_skbuff[i];
                if (sk_buff) {
                        dma_unmap_single(device, ring->RxDescPhyAddr[i], ring->num_rx_desc, DMA_FROM_DEVICE);
                        dev_kfree_skb(sk_buff);
			sk_buff = NULL;
                        rtl8125_make_unusable_by_asic(tp, rtl8125_get_rxdesc(tp, ring->RxDescArray, i));                       
                }
        }
}
                ...
                u32 start, u32 end, u8 in_intr)
{
        ...
        for (cur = start; end - cur > 0; cur++) {
                ...
                if (ring->Rx_skbuff[i])
                        continue;
                ret = rtl8125_alloc_rx_skb(tp, ring, ring->Rx_skbuff + i,
                        rtl8125_get_rxdesc(tp, ring->RxDescArray, i), i, in_intr);
                if (ret < 0)
                        break;
        }
        return cur - start;
...
void
rtl8125_mark_as_last_descriptor(struct rtl8125_private *tp, struct RxDesc *desc)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static int
rtl8125_init_ring(struct rtl8125_private *tp)
{
        int i;
...
#endif //ENABLE_PAGE_REUSE
                if (rtl8125_rx_fill(tp, ring, 0, NUM_RX_DESC, 0) != NUM_RX_DESC)
                        goto err_out;

                rtl8125_mark_as_last_descriptor(tp, rtl8125_get_rxdesc(tp, ring->RxDescArray, 
			NUM_RX_DESC - 1));
        }
...
static inline void
rtl8125_unmap_tx_skb(struct device *device, struct ring_info *tx_skb, struct TxDesc *desc)
{
...
static void
rtl8125_tx_clear_range(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring,
...
static inline void rtl8125_schedule_linkchg_work(struct rtl8125_private *tp)
{
...
static void
_rtl8125_wait_for_quiescence(struct rtl8125_private *tp)
{
...
/*
static void
rtl8125_wait_for_quiescence(struct rtl8125_private *tp)
{
        //suppress unused variable
        (void)(tp);

        _rtl8125_wait_for_quiescence(tp);
...
static void rtl8125_reset_task(struct work_struct *work)
{
        ...
        for (int i = 0; i < tp->num_rx_rings; i++) {
                struct rtl8125_rx_ring *ring = &tp->rx_ring[i];
                for (u32 entry = 0; entry < NUM_RX_DESC; entry++) {
...
static inline u32 rtl8125_get_txd_opts1(struct rtl8125_tx_ring *ring, u32 opts1,
...
static __always_inline int
rtl8125_xmit_frags(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring,
                   struct sk_buff *skb, const u32 *opts, unsigned int entry)
{
        ...
        for (frags = 0; frags < nr_frags; frags++) {
                const skb_frag_t *frag = info->frags + frags;
                u32 opts1, len = skb_frag_size(frag);
...
static __always_inline __be16 get_protocol(struct sk_buff *skb)
{
...
static __always_inline bool rtl8125_skb_pad(struct sk_buff *skb)
{
...
static inline bool rtl8125_skb_is_udp(struct sk_buff *skb)
{
	int no = skb_network_offset(skb);
	struct ipv6hdr *i6h, _i6h;
	struct iphdr *ih, _ih;
...
static inline unsigned int rtl8125_udp_padto(struct sk_buff *skb)
{
	unsigned int padto = 0, len = skb->len;

	if (len < 128 + MIN_PATCH_LEN && //rtl8125_require_pad_ptp_pkt(tp) &&
...
rtl8125_tso_csum_v2(struct rtl8125_private *tp, struct sk_buff *skb, u32 *opts)
{
	...
	if (mss) {
		...
		unsigned int padto = rtl8125_udp_padto(skb);
...
static __always_inline bool rtl8125_tx_slots_avail(struct rtl8125_private *tp,
...
static __always_inline void
rtl8125_doorbell(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring)
{
#ifdef ENABLE_TX_NO_CLOSE
        if (tp->HwSuppTxNoCloseVer == 3)
...
static netdev_tx_t
rtl8125_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
        struct rtl8125_private *tp = netdev_priv(dev);
        struct rtl8125_tx_ring *ring = &tp->tx_ring[skb->queue_mapping];
        //unsigned int   bytecount;
        //unsigned short gso_segs;
        struct ring_info *last;
        unsigned int entry = ring->cur_tx % NUM_TX_DESC;
        ...
        //if (unlikely(!rtl8125_tso_csum(skb, tp, dev->features, opts, &bytecount, &gso_segs)))
        if (unlikely(!rtl8125_tso_csum_v2(tp, skb, opts)))
                goto err_dma_0;

        frags = rtl8125_xmit_frags(tp, ring, skb, opts, entry);
        if (unlikely(frags < 0))
                goto err_dma_0;
        if (frags) {
                len = skb_headlen(skb);
        ...
        /* map tx */
        mapping = dma_map_single(tp->device, skb->data, len, DMA_TO_DEVICE);
        if (unlikely(dma_mapping_error(tp->device, mapping))) {
                if (unlikely(net_ratelimit()))
        ...
        entry = (entry + frags) % NUM_TX_DESC;   /* set skb to last fragment */

        /* end of tx mapping*/

        /* set skb to last fragment */
        last = &ring->tx_skb[entry];
        last->skb = skb;
        last->gso_segs = 1;
        last->bytecount = skb->len;
        ...
        if (unlikely(stop_queue)) {
                ...
                if (rtl8125_tx_slots_avail(tp, ring))
                        netif_start_subqueue(dev, skb->queue_mapping);
        }
out:
        return NETDEV_TX_OK;
err_dma_1:
...
static __always_inline int
rtl8125_tx_interrupt_noclose(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring, int budget)
{
	...
	tx_desc_closed = rtl8125_get_desc_closed(tp, ring);
        tx_left = READ_ONCE(ring->cur_tx) - dirty_tx;
	tx_left = min(tx_left, tx_desc_closed);
        ...
        if (total_packets) {
                netdev_tx_completed_queue(txring_txq(ring), total_packets, total_bytes);
                dev->stats.tx_bytes += total_bytes;
                dev->stats.tx_packets+= total_packets;
        }
...
static __always_inline int
rtl8125_tx_interrupt_close(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring, int budget)
{
        unsigned int total_bytes = 0, total_packets = 0;
        struct net_device *dev = tp->dev;
       	struct sk_buff *skb;
...
static inline int
rtl8125_tx_interrupt(struct rtl8125_private *tp, struct rtl8125_tx_ring *ring, int budget)
{
#ifdef ENABLE_TX_NO_CLOSE
        return rtl8125_tx_interrupt_noclose(tp, ring, budget);
#else
        return rtl8125_tx_interrupt_close(tp, ring, budget);
#endif
}

static inline int
rtl8125_tx_interrupt_with_vector(struct rtl8125_private *tp, const int message_id, int budget)
{
        int idx = 0;

        if (tp->HwCurrIsrVer == 3 || tp->HwCurrIsrVer == 4) {
                if (message_id < tp->num_tx_rings) {
                        idx = message_id;
                        goto tx_interrupt;
                } 
...
#ifdef ENABLE_MULTIPLE_TX_QUEUE
        if ((tp->HwCurrIsrVer != 5 && message_id == 18) || message_id == 17) {
                        idx = 1;
                        goto tx_interrupt;
                }
#endif
...
tx_interrupt:
        return rtl8125_tx_interrupt(tp, &tp->tx_ring[idx], budget);
...
/*
        ...
        return (idx < 0) ? 0 : rtl8125_tx_interrupt(tp, &tp->tx_ring[idx], budget);
*/
}
...
static __always_inline int
rtl8125_is_eop(struct rtl8125_private *tp, u32 status)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static inline bool
rtl8125_rx_v1_csum(struct RxDesc *desc)
{
...
static inline bool
rtl8125_rx_v3_csum(struct RxDescV3 *descv3)
{
...
static inline bool
rtl8125_rx_v4_csum(struct RxDescV4 *descv4)
{
...
static inline bool
rtl8125_rx_csum(u8 rxDescType, struct RxDesc *desc)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (rxDescType == RX_DESC_RING_TYPE_3)
                return rtl8125_rx_v3_csum((struct RxDescV3 *)desc);
#endif
#ifdef ENABLE_RSS_SUPPORT        
        if (likely(rxDescType == RX_DESC_RING_TYPE_4))
                return rtl8125_rx_v4_csum((struct RxDescV4 *)desc);
#endif
        return rtl8125_rx_v1_csum(desc);
}

static inline void
rtl8125_rx_skb(struct rtl8125_private *tp, struct sk_buff *skb, u32 ring_index)
{
#ifdef CONFIG_R8125_NAPI
...
static inline int
rtl8125_check_rx_desc_error(struct net_device *dev, struct rtl8125_private *tp, u32 status)
{
#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (tp->InitRxDescType == RX_DESC_RING_TYPE_3) {
...
static void
rtl8125_put_rx_buffer(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring,
                      u32 cur_rx, struct rtl8125_rx_buffer *rxb)
{
        struct rtl8125_rx_buffer *nrxb;
        struct page *page = rxb->page;
        u32 entry;

        entry = ring->dirty_rx % NUM_RX_DESC;
        ...
        dma_sync_single_range_for_device(tp->device, nrxb->dma, nrxb->page_offset,
                                         RX_BUF_SIZE, DMA_FROM_DEVICE);

        rtl8125_map_to_asic(tp, ring, rtl8125_get_rxdesc(tp, ring->RxDescArray, entry),
                            nrxb->dma + nrxb->page_offset, RX_BUF_SIZE, entry);
...
static int
rtl8125_rx_interrupt(struct rtl8125_private *tp, struct rtl8125_rx_ring *ring, int budget)
{
        u32 cur_rx, rx_left, delta, count = 0;
        struct net_device *dev = tp->dev;
        struct device *device = tp->device;
#ifdef ENABLE_PAGE_REUSE
        struct rtl8125_rx_buffer *rxb;
        ...
        u64 rx_buf_phy_addr;
#endif //ENABLE_PAGE_REUSE
        u32 total_rx_multicast_packets = 0, total_rx_bytes = 0, total_rx_packets = 0;

        //assert(dev != NULL); assert(tp != NULL);
        ...
        rx_left = min(rx_left, (u32)budget);

        for (; rx_left > 0; rx_left--, cur_rx++) {
                u32 pkt_size, entry = cur_rx % NUM_RX_DESC;
                struct RxDesc *desc = rtl8125_get_rxdesc(tp, ring->RxDescArray, entry);
                struct sk_buff *skb;
#ifdef ENABLE_PTP_SUPPORT
                ...
                const void *rx_buf;
#endif //!ENABLE_PAGE_REUSE
                u32 status = le32_to_cpu(rtl8125_rx_desc_opts1(tp, desc));
                if (status & DescOwn)
                        break;
...
#ifdef ENABLE_PTP_SUPPORT
                if (likely(tp->EnablePtp)) {
                        desc_type = rtl8125_rx_desc_type(status);
                        if (desc_type == RXDESC_TYPE_NEXT && rx_left > 0) {
                                unsigned int entry_next = ++cur_rx % NUM_RX_DESC;
                                struct RxDescV3 *desc_next = (struct RxDescV3 *)
                                        rtl8125_get_rxdesc(tp, ring->RxDescArray, entry_next);
                                u32 status_next = le32_to_cpu(desc_next->RxDescNormalDDWord4.opts1);
                                rx_left--;
...
map_and_mark_to_asic:
                                rtl8125_map_and_mark_to_asic(tp, (struct RxDesc *)desc_next,
                                                ring->RxDescPhyAddr[entry_next]);
                                /*
                                rtl8125_set_desc_dma_addr(tp, (struct RxDesc *)desc_next,
                                                                ring->RxDescPhyAddr[entry_next]);
                ...
                if (!skb) {
                        ...
                        skb_add_rx_frag(skb, skb_shinfo(skb)->nr_frags, rxb->page,
                                        rxb->page_offset, pkt_size, RX_BUF_SIZE / 2);
#ifdef ENABLE_PTP_SUPPORT
                if (desc_type == RXDESC_TYPE_PTP)
                ...
                dma_sync_single_range_for_cpu(device, rxb->dma, rxb->page_offset,
                                              RX_BUF_SIZE, DMA_FROM_DEVICE);
#else //ENABLE_PAGE_REUSE
                skb = napi_alloc_skb(&tp->r8125napi[ring->index].napi, pkt_size);
                if (!skb) {
                        //netdev_err(tp->dev, "Failed to allocate RX skb!\n");
                        goto drop_packet;
                }
                ...
                dma_sync_single_for_cpu(device, rx_buf_phy_addr, RX_BUF_SIZE, DMA_FROM_DEVICE);
                rx_buf = ring->Rx_skbuff[entry]->data;
                prefetch(rx_buf);
       		skb_copy_to_linear_data(skb, rx_buf, pkt_size);

                dma_sync_single_for_device(device, rx_buf_phy_addr, RX_BUF_SIZE, DMA_FROM_DEVICE);
#endif //ENABLE_PAGE_REUSE
...
#endif
                if (rtl8125_rx_csum(tp->InitRxDescType, desc))
...
release_descriptor:
                ...
                case RX_DESC_RING_TYPE_4:
                        rtl8125_set_desc_dma_addr(tp, desc, ring->RxDescPhyAddr[entry]);
                        wmb();
                        break;
                }
                rtl8125_mark_to_asic(tp, desc);
...
drop_packet:
        ...
        delta = rtl8125_rx_fill(tp, ring, ring->dirty_rx, ring->cur_rx, 1);
        if (!delta && count && netif_msg_intr(tp))
                printk(KERN_INFO "%s: no Rx buffer allocated\n", dev->name);
        ring->dirty_rx += delta;

        dev->stats.rx_bytes += total_rx_bytes;
        dev->stats.rx_packets += total_rx_packets;
        dev->stats.multicast += total_rx_multicast_packets;
...
static irqreturn_t rtl8125_interrupt(int irq, void *dev_instance)
{
        struct rtl8125_private *tp = ((struct r8125_napi *) dev_instance)->priv;
        u32 status = RTL_R32(tp, tp->isr_reg[0]);

        //if (unlikely(!(tp->features & (RTL_FEATURE_MSI | RTL_FEATURE_MSIX)))) {
        if (unlikely(tp->irq_nvecs < 1)) {  // Implies RTL_FEATURE_MSI | RTL_FEATURE_MSIX
        ...
        if (status & tp->intr_mask || tp->keep_intr_cnt-- > 0) {
                ...
                if (likely(napi_schedule_prep(&tp->r8125napi[0].napi)))
                        __napi_schedule(&tp->r8125napi[0].napi);
        ...
        if (status & tp->intr_mask || tp->keep_intr_cnt-- > 0) {
                ...
                for (int i = 0; i < tp->num_tx_rings; i++)
                        rtl8125_tx_interrupt(tp, &tp->tx_ring[i], ~(u32)0);

                rtl8125_rx_interrupt(tp->dev, tp, &tp->rx_ring[0], ~(u32)0);
...
static irqreturn_t rtl8125_interrupt_msix(int irq, void *dev_instance)
{
        ...
        do {
...
#ifdef CONFIG_R8125_NAPI
                if (likely(napi_schedule_prep(&r8125napi->napi))) {
                        rtl8125_disable_hw_interrupt_v2(tp, message_id);
                        __napi_schedule(&r8125napi->napi);
                } else if (netif_msg_intr(tp))
                ...
                if (message_id < tp->num_rx_rings) {
                        rtl8125_rx_interrupt(tp->dev, tp, &tp->rx_ring[message_id], budget);
                }

...
static void rtl8125_ptp_tx_work(struct work_struct *work)
{
        ...
        else /* reschedule to check later */
...
static long rtl8125_ptp_create_clock(struct rtl8125_private *tp)
{
        ...
        //if (tp->HwSuppPtpVer == 0) { tp->ptp_clock = NULL; return -EOPNOTSUPP; }
...
void rtl8125_ptp_init(struct rtl8125_private *tp)
{
        ...
        netif_info(tp, drv, tp->dev, "r8125: PTP initialized (%s)\n", 
                tp->ptp_master_mode? "master": "slave");
        return;
}
...
int rtl8125_set_tstamp(struct rtl8125_private *tp, struct ifreq *ifr)
{
        ...
        netif_info(tp, drv, tp->dev, "rtl8125_set_tstamp: rx_filter:%d\n", config.rx_filter);
